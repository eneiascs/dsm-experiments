...
public class SimulationServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// resp.getWriter().println(String.format("<h3>find by id:
		// %s</h3>",experimentDesignProvider.findById("59ac3ef9857aba00016e6bcf")));
		// resp.getWriter().println(String.format("<h3>find by id:
		// %s</h3>",experimentExecutionClient.findById("59ac2950781b58602b76a596")));
		//
		...
		experimentExecutionClient.findAll().forEach((execution) -> {
			if (!execution.getExecutionStatus().name().equals(ExecutionStatusDTO.FINISHED.name())) {
				// execution.setCpu(ThreadLocalRandom.current().nextLong(1L,
				// 1000L));
				// execution.setTime(ThreadLocalRandom.current().nextLong(1L,
				// 1000L));
				// execution.setMemory(ThreadLocalRandom.current().nextLong(1L,
				// 1000L));
		...
		// MappingService
		// mappingService=MappingServiceFactory.get("/home/eneiascs/eclipse/java-neon/eclipse/dslforge/ReanaSpl/src-gen/preconditions.json");
		...
		// // ExperimentDesignDTO
		// exp=experimentDesignProvider.findById("59a9b26e781b581e1022856b");
		// ExperimentDesignDTO exp2=experimentDesignProvider.findByJobId("Fri
		// Sep 01 19:18:06 GMT 2017");
		//
		...
		// }
		// List<ExperimentExecutionDTO> list=new
		// ArrayList<ExperimentExecutionDTO>();
		//
		...
		// List<ExperimentExecutionDTO>
		// execs=experimentExecutionClient.findAll();
		...
		// "+experimentExecutionClient.findById("59a45329857aba0001cc9808"));
		//
